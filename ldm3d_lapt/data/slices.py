...
class TargetSliceDataset(Dataset):
    def __init__(self, dataroot, phase, transform, lesion=False, threshold=0):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        if dataroot_server and (not os.path.exists(dwi_path) or not os.path.exists(adc_path) or not os.path.exists(mask_path) or not os.path.exists(lesion_path) or not os.path.exists(dwi_stats_path) or not os.path.exists(adc_stats_path)):
            dwi, adc, mask, lesion, dwi_mean, dwi_std, adc_mean, adc_std, patient_id, study_date, series_id, patient_dir, i = self._single_scan(dataroot, patient_id, study_date, series_id, i)
        else:
            try:
        ...
        data = self.transform({'dwi': dwi, 'adc': adc, 'mask': mask, 'lesion': lesion, 'dwi_mean': dwi_mean, 'dwi_std': dwi_std, 'adc_mean': adc_mean, 'adc_std': adc_std})
    ...
    def _single_scan(self, dataroot, patient_id, study_date, series_id, i):
        ...
        if dataroot_server and (not os.path.exists(dwi_path) or not os.path.exists(adc_path) or not os.path.exists(mask_path) or not os.path.exists(lesion_path)):
        ...
        return [dwi_slice, adc_slice, mask_slice, lesion_slice, dwi_mean, dwi_std, adc_mean, adc_std, patient_id, study_date, series_id, patient_dir, i]
...
class TargetSliceTrain(TargetSliceDataset):
...
class TargetSliceTest(TargetSliceDataset):
    def __init__(self, dataroot):
