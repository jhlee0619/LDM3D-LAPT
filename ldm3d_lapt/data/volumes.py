...
from .transforms import get_train_transforms_3d, get_val_transforms_3d, get_train_bigaug_transforms_3d, get_val_bigaug_transforms_3d
...
class SegVolumeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if dataroot_server and (not os.path.exists(dwi_path) or not os.path.exists(adc_path) or not os.path.exists(mask_path)):
...
class SegVolumeTrain(SegVolumeDataset):
    def __init__(self, dataroot, bigaug=False):
        print(f'bigaug: {bigaug}')
        transform = get_train_transforms_3d() if not bigaug else get_train_bigaug_transforms_3d()
        super().__init__(dataroot, phase='train', transform=transform)




class SegVolumeTest(SegVolumeDataset):
    def __init__(self, dataroot, bigaug=False):
        transform = get_val_transforms_3d() if not bigaug else get_val_bigaug_transforms_3d()
