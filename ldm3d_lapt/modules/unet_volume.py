...
from ldm3d_lapt.modules.attention import SpatialTransformer
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1, temporal_length=32):
        ...
        if use_conv:
            ...
            self.conv_tem = PseudoConv3d(self.out_channels, self.out_channels, 3, temporal_length=temporal_length)
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        x = x + h_tem
        x = rearrange(x, '(b h w) c d -> (b d) c h w', h = spatial[0], w = spatial[1])
        return x
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    ...
                    PseudoConv3d(model_channels, model_channels, 3, temporal_length=self.temporal_length)
                )
            ]
        )
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                            ...
                            ch, conv_resample, dims=dims, out_channels=out_ch, temporal_length=self.temporal_length
                        )
                    )
                )
                ch = out_ch
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample(ch, conv_resample, dims=dims, out_channels=out_ch, temporal_length=self.temporal_length)
                    )
                    ds //= 2
        ...
        self.out_tem = PseudoConv3d(out_channels, out_channels, 3, temporal_length=self.temporal_length)
