...
class AttnBlock3D(nn.Module):
    ...
    def forward(self, x):
        ...
        w_ = w_.permute(0,2,1)   # b,dhw,dhw
        h_ = torch.bmm(v,w_)     # b,c,dhw
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks+1):
                ...
                block_in = block_out
                if curr_res in attn_resolutions:
                    attn.append(AttnBlock3D(block_out))
            up = nn.Module()
            up.block = block
            up.attn = attn
            if i_level != 0:
                up.upsample = Upsample3D(block_out, resamp_with_conv)
                curr_res = curr_res * 2
            self.up.insert(0, up) # prepend to get correct order
