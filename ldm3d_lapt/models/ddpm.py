...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # check that the conditioning stage is being updated (every 100 steps)
        if self.cond_stage_trainable and hasattr(self, 'cond_stage_params_snapshot') and batch_idx % 100 == 0:
            ...
            for name, param in self.cond_stage_model.named_parameters():
                if name in self.cond_stage_params_snapshot:
                    ...
                    if diff > 1e-6:  # detect change with a small threshold
                        param_changed = True
                        if diff > max_diff:
                            max_diff = diff
    ...
    def log_images(self, batch, N=8, n_row=2, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log["conditioning"] = c
        log["inputs"] = normalize_for_vis(x, dwi_mean=dwi_mean, dwi_std=dwi_std,
        ...
        if sample:
            ...
            log["samples"] = normalize_for_vis(samples, dwi_mean=dwi_mean, dwi_std=dwi_std,
                                               adc_mean=adc_mean, adc_std=adc_std)
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            ...
            # # keep the initial weights so parameter updates can be tracked
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            log["samples"] = normalize_for_vis(x_samples, dwi_mean=dwi_mean, dwi_std=dwi_std,
                                               adc_mean=adc_mean, adc_std=adc_std)
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                ...
                log["samples_x0_quantized"] = normalize_for_vis(x_samples, dwi_mean=dwi_mean, dwi_std=dwi_std,
                                                               adc_mean=adc_mean, adc_std=adc_std)
